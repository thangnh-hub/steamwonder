...
@section('content')
    ...
    <section class="content">
        ...
        <div class="box">
            <div class="box-header text-center">
                <h3 class="box-title">CHÀO MỪNG {{ $student->name }} - CCCD:{{$student->json_params->cccd}} ĐẾN VỚI BÀI {{$exam_session->title}}
                </h3>
                @php
                    $now = new DateTime();
            ...
            <div class="box-body">
                ...
                @endif
                <form role="form" action="{{ route('test_iq.student.answer') }}" method="POST" id="form_test_iq">
                    @csrf
                    <div class="row">
                        <div class="col-lg-12">
                                    ...
                                    <div class="box-body more_question">
                                        <div class="nav-tabs-custom">
                                            ...
                                            <div class="tab-content">
                                                @foreach ($question->shuffle() as $key => $row)
                                                    @switch($row_topic->type)
                                                        ...
                                                        @case('order_table')
                                                            ...
                                                            @php
                                                                $shuffledAnswers = (array) $row->json_params->answer;
                                                                $ques = strip_tags($row_topic->content);
                                                                $arr_ques = explode(' - ', $ques);

                                                            @endphp
                                                            <div class="d-flex-wap more_answer" style="margin-bottom: 30px"
                                                                data-id="{{ $row->id }}"
                                                                data-count="{{ count($shuffledAnswers) }}">
                                                                @foreach (collect(explode('-', strip_tags($row_topic->content)))->shuffle() as $val)
    ...
    @push('script')
        <script>
            $('.btn_quiz_choice').click(function() {
                ...
                if (!$(this).hasClass('active')) {
                    if (_active < _count) {
                        ...
                        $(this).after('<input class="checked_val" type="hidden" name="answer[' + _id + '][]" value="' +
                            _answer + '">');
                    }
                } else {
            ...
            var session_check = sessionStorage.getItem('check_iq');
            ...
            function checkStatus() {
                var check = sessionStorage.getItem('check_iq');
                if (check == 'off') {
                    sessionStorage.removeItem('check_iq');
                    location.reload();
                }
                return;
            }
