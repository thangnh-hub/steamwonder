                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>@lang('Mã chính sách') <small class="text-red">*</small></label>
                                <input type="text" class="form-control" name="code" id="code"
                                    placeholder="@lang('Mã chính sách')" value="{{ $detail->code ?? old('code') }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>@lang('Tên chính sách') <small class="text-red">*</small></label>
                                <input type="text" class="form-control" name="name" id="class_name"
                                    placeholder="@lang('Tên chính sách')" value="{{ $detail->name ?? old('name') }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                    ...
                                    <li class="d-flex-wap item_service">
                                        ...
                                        <select name="json_params[services][{{ $item_service->id }}][type]"
                                            ...
                                            @foreach ($type as $item)
                                                <option value="{{ $item }}" {{isset($detail->json_params->services->{$item_service->id}->type) && $detail->json_params->services->{$item_service->id}->type == $item ?'selected':''}}>{{ __($item) }}</option>
                                            @endforeach
                                        </select>
