...
@section('content')
                                ...
                                <ul class="nav nav-tabs">
                                    <li class="active">
                                        <a href="#tab_1" data-toggle="tab">
                                            <h5>@lang('Thông tin học sinh') <span class="text-danger">*</span></h5>
                                        </a>
                                    </li>
                                    <li class="">
                                        <a href="#tab_2" data-toggle="tab">
                                            <h5>@lang('Người thân của bé')</h5>
                                        </a>
                                    </li>
                                    <li class="">
                                        <a href="#tab_3" data-toggle="tab">
                                            <h5>@lang('Dịch vụ đã đăng ký')</h5>
                                        </a>
                                    </li>
                                    <li class="">
                                        <a href="#tab_5" data-toggle="tab">
                                            <h5>@lang('CT Kh.Mãi được áp dụng')</h5>
                                        </a>
                                    </li>
                                    <li class="">
                                        <a href="#tab_4" data-toggle="tab">
                                            <h5>@lang('Quản lý TBP HSM')</h5>
                                        </a>
                                    </li>
                                                        ...
                                                        @foreach ($detail->studentReceipt as $row)
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @if ($admin_auth->id == $row->admin_created_id && $row->type_receipt == 'new_student')
                                                        ...
                                                        <tr>
                                                            <td class="text-center">
                                                                <input type="radio" class="radio_promotion"
                                                                    {{ in_array($row->id, $promotion_active->pluck('promotion_id')->toArray()) ? 'disabled' : '' }}
    ...
    <div class="modal fade" id="addParentModal" tabindex="-1" role="dialog" aria-labelledby="addParentModalLabel"
        ...
        <div class="modal-dialog modal-full" role="document">
            <form id="formStudentAddParent" action="{{ route('admission.student.addParent', $detail->id) }}"
                method="POST">
                @csrf
    ...
    <div data-backdrop="static" class="modal fade" id="addServiceModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-full" role="document">
            <form id="submitstudentaddService" action="{{ route('admission.student.addService', $detail->id) }}"
                method="POST">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                                ...
                                <tbody>
                                    @foreach ($unregisteredServices as $service)
                                        <tr>
                                            ...
                                            <td>
                                                <select style="width:100%"
                                                    name="services[{{ $service->id }}][payment_cycle_id]"
                                                    class="form-control select2">
                                                    @foreach ($list_payment_cycle as $payment_cycle)
                                                        <option {{ $payment_cycle->is_default == 1 ? 'selected' : '' }}
                                                            value="{{ $payment_cycle->id }}">
                                                            {{ $payment_cycle->name ?? '' }}</option>
                                                    @endforeach
                                                </select>
    ...
    <div data-backdrop="static" class="modal fade" id="reincarnationModal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <form id="formRenew" action="{{ route('admission.receipt.calculateStudent') }}" method="POST">
                @csrf
    ...
    <script>
        ...
        $('.delete_student_service').click(function(e) {
            if (confirm('Bạn có chắc chắn muốn xóa dịch vụ này khỏi học sinh ?')) {
                ...
                let url = "{{ route('admission.delete_student_service') }}/";
                $.ajax({
                    type: "GET",
                    url: url,
                    data: {
        ...
        $('#btnUpdateService').click(function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('admission.student.updateService.ajax') }}",
                data: {
        ...
        $('.btn_delete_receipt').click(function() {
            ...
            if (confirm("Bạn có chắc chắn muốn xóa phiếu thu này?")) {
                $.ajax({
                    type: "GET",
                    url: "{{ route('admission.student.deleteReceipt') }}",
                    data: {
