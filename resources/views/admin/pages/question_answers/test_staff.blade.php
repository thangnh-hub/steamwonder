    ...
    <section class="content">
        ...
        <div class="box">
            <div class="box-header">
                <h3 class="box-title">@lang('List of questions to be answered')</h3>
            </div>
            <div class="box-body">
    ...
    <script>
        ...
        $(document).on('click', '.add_question', function() {
            ...
            var _html = `<div class="box-body more_question">
                        <div class="nav-tabs-custom">
                            <ul class="nav nav-tabs">
                                <li class="active" style="display:flex;padding: 10px;">

                                    <a href="#tab_` + (_count + 1) + `" data-toggle="tab" style="width: 1110px;">
                                        <input type="text" class="form-control" name="list[` + (_count + 1) +
                `][question]"
                                            placeholder="@lang('Question')"
                                            required>
                                    </a>
                                    <button onclick="_delete_question(this)" type="button" class="btn btn-sm btn-danger" data-question="` + (_count + 1) + `">Xóa</button>
                                </li>
                            </ul>
                            <div class="tab-content">
                ...
                `][json_params][answer][` + (_count_an) + `][value]"
                                        ...
                                        <div class="col-md-2">
                                            <select name="list[` + (_count + 1) + `][json_params][answer][` + (
                    _count_an) + `][boolean]" class="form-control" style="width: 100%;">
                                                    <option value="">@lang('Please select')</option>
                                                    @foreach ($booleans as $b => $val)
                                                        <option value="{{ $b }}">@lang($booleans[$b])</option>
                                                    @endforeach
                                            </select>
                                        </div>
                                    </div>
        ...
        $(document).on('click', '.add_answer', function() {
            ...
            var _html = `<div class="d-flex-wap more_answer">
                ...
                `][json_params][answer][` + (_count_an + 1) + `][value]"
                        ...
                        <div class="col-md-2">
                            <select name="list[` + (_count) + `][json_params][answer][` + (_count_an + 1) +
                `][boolean]" class="form-control" style="width: 100%;">
                                    <option value="">@lang('Please select')</option>
                                    @foreach ($booleans as $b => $val)
                                        <option value="{{ $b }}">@lang($booleans[$b])</option>
                                    @endforeach
                        ...
                        <button onclick="_delete_answer(this)" type="button" class="btn btn-sm btn-danger" data-answer="` + (_count_an + 1) + `" data-question="` + (_count) + `">Xóa</button>
                    </div>`;
