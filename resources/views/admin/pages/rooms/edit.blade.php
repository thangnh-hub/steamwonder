        ...
        <form role="form" action="{{ route(Request::segment(2) . '.update', $detail->id) }}" method="POST">
            ...
            <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="datetime-local" class="form-control" name="start_date"
                                                        ...
                                                        value="{{ $detail->start_date ?? old('start_date') }}">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title">@lang('Area')</h3>
                        </div>
                        <div class="box-body">
                            <div class="form-group">
                    ...
                    <div class="box box-primary">
                        <div class="box-header with-border">
                            <h3 class="box-title">@lang('Internal')</h3>
                        </div>
                        <div class="box-body">
                            <div class="form-control">
                                <label>
                                  <input type="radio" name="is_internal" value="1"
                                    {{ $detail->is_internal == '1' ? 'checked' : '' }}>
                                  <small>@lang('true')</small>
                                </label>
                                <label>
                                  <input type="radio" name="is_internal" value="0" class="ml-15"
                                    {{ $detail->is_internal == '0' ? 'checked' : '' }}>
    ...
    <script>
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
          ...
          headerToolbar: {
            ...
            center: 'title',
            right: 'dayGridMonth,timeGridWeek,timeGridDay,multiMonthYear' // user can switch between the two
          },

          events: [
            @foreach($list_schedule as $schel)
            {
                title: '{{ $schel->class->name??"" }}',
                start: '{{ $schel->date??"" }}T{{ $schel->period->start_time??"" }}',
                end: '{{ $schel->date??"" }}T{{ $schel->period->end_time??"" }}',
                // url: '#'
            },
          @endforeach
        ...
        eventClick: function(info) {
            ...
            // Lấy giờ và phút kết thúc từ đối tượng sự kiện
            var endHour = info.event.end.getHours();
            var endMinute = info.event.end.getMinutes();

            var nDate = info.event.start.toISOString().split('T')[0];
            alert(`Lớp `+title+` bắt đầu từ `+startHour+`:`+startMinute+` đến `+endHour+`:`+endMinute+` ngày `+nDate+` `);
        },
        locale: 'vi',

        dayHeaderFormat: {
            weekday: 'long', // Hiển thị tên đầy đủ của ngày trong tuần
