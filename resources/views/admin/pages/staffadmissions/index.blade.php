            ...
            <form action="{{ route(Request::segment(2) . '.index') }}" method="GET">
                <div class="box-body">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <label>@lang('Direct manager')</label>
                                <select name="parent_id" id="parent_id" class="form-control select2" style="width: 100%;">
                                    <option value="">@lang('Please select')</option>
                                    @foreach ($rows as $key => $value)
                                        <option value="{{ $value->id }}"
                                            {{ isset($params['parent_id']) && $value->id == $params['parent_id'] ? 'selected' : '' }}>
                ...
                @else
                    <table class="table table-hover table-bordered">
                        <thead>
                            <tr>
                                <th>@lang('Email')</th>
                                <th>@lang('Staff admission code')</th>
                        ...
                        <tbody>
                            @foreach ($rows as $row)
                                @php
                                    $direct_manager = \App\Models\StaffAdmission::find($row->parent_id ?? '');
                                @endphp
                                <form action="{{ route(Request::segment(2) . '.destroy', $row->id) }}" method="POST"
                                    ...
                                    <tr class="valign-middle">
                                        ...
                                        <td>
                                            <a class="btn btn-sm" data-toggle="tooltip" title="@lang('Xem chi tiết')"
                                        ...
                                        <td>
                                            @isset($direct_manager->id)
                                                <a
                                                    href="{{ route(Request::segment(2) . '.edit', $direct_manager->id) }}">{{ $direct_manager->name ?? '' }}</a>
                                            @endisset
                                        </td>
                                        <td>
                                            <a
                                                href="{{ route('students.index', ['admission_id' => $row->id]) }}">@lang('DS Student Admissions')</a>
                                        </td>
                                        <td>
                                            @lang($row->status)
                                        </td>
