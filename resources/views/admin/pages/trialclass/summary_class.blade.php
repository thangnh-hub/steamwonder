            ...
            <form action="{{ route(Request::segment(2) . '.index') }}" method="GET">
                <div class="box-body">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <label>@lang('Estimated End Date of the Course') </label>
                    ...
                    <table class="table table-hover table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th>@lang('Lesson studied')</th>
                                <th>@lang('Teacher')</th>
                                <th>@lang('Estimated End Date of the Course')</th>
                                <th>@lang('Action')</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($rows as $row)
                                @php
                                    ...
                                    $lesson_studied = \App\Models\Schedule::where('class_id',$row->id)->where('status', \App\Consts::SCHEDULE_STATUS['dadiemdanh'])->get()->count();
                                @endphp
                                    <tr class="valign-middle">
                                        <td>
                                            {{-- <a
                                                href="{{ route('areas.edit', $row->area_id) }}">{{ $row->area->name ?? '' }}</a> --}}
                                                {{ $row->area->name ?? '' }}
                                        </td>
                                        <td>
                                            <strong
                                        ...
                                        <td>
                                            {{-- <a
                                                href="{{ route('levels.edit', $row->level_id) }}">{{ $row->level->name ?? '' }}</a> --}}
                                                {{ $row->level->name ?? '' }}
                                        </td>
                                        <td>
                                            {{ $row->syllabus->lesson_min ?? 0 }}
                                        </td>
                                        <td>
                                            {{ $lesson_studied ?? 0 }}
                                        </td>
                                        ...
                                        <td>
                                            {{ date('d-m-Y', strtotime($row->schedules->last()->date)) }}
                                        </td>
                                        <td><a href="{{ route('summary_attendance.index', ['class_id' => $row->id]) }}">@lang('Class attendance details')</a></td>
                                    </tr>
                            @endforeach
                        </tbody>
                    </table>
