                ...
                <div class="col-md-12">
                    @isset($question->json_params->answer)
                        <div class="list_answer">
                            <label>Đáp án:</label>
                            @foreach($question->json_params->answer as $k=> $answer)
                                <div class="d-flex-wap more_answer">
                                    <div class="col-md-6 pl-0">
                                        <div class="form-group {{ $loop->index>0 ? "input-group" :""}}">
                                            <input type="text" name="json_params[answer][{{ $k }}][value]" class="form-control" placeholder="Đáp án" value="{{ $answer->value??"" }}">
                                            @if($loop->index>0)
                                    ...
                                    <div class="col-md-1">
                                        <input type="checkbox" name="json_params[answer][{{ $k }}][boolean]" value="{{ (isset( $answer->boolean) && $answer->boolean==1 )? 1:0 }}"{{ (isset( $answer->boolean) && $answer->boolean==1 )? 'checked':"" }}  onchange="updateCheckboxValue(this)" >
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        <button onclick="add_answer_choice(this)" class="form-group btn btn-primary mb-2" type="button"><i class="fa fa-plus"></i>
