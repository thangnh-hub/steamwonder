                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="keyword" placeholder="@lang('Từ khóa ...')"
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <label>@lang('Người thực hiện')</label>
                                <select name="person_id" class="form-control select2" style="width: 100%;">
                                    <option value="">@lang('Please select')</option>
                                    @foreach ($persons as $val)
                                        <option value="{{ $val->id }}"
                                            {{ isset($params['person_id']) && $params['person_id'] == $val->id ? 'selected' : '' }}>
                                            {{ __($val->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                @else
                    <table class="table table-hover table-bordered table-responsive">
                        <thead>
                            <tr>
                                <th>@lang('STT')</th>
                                <th>@lang('Kỳ kiểm kê')</th>
                                <th>@lang('Cơ sở')</th>
                                <th>@lang('Kho')</th>
                                <th>@lang('Phòng ban')</th>
                                <th>@lang('Người kiểm kê')</th>
                                <th>@lang('Ngày kiểm kê')</th>
                        ...
                        <tbody>
                            @foreach ($rows as $row)
                                <tr class="valign-middle">
                                    ...
                                    <td>
                                        {{ $row->date_received != '' ? date('d/m/Y', strtotime($row->date_received)) : '---' }}
                                    </td>
                                    <td>
                                        {{ __($row->status) }}
                                    </td>
                                    <td class="d-flex-wap">
                                        @if ($row->status == $status_inventory['Approve'])
                                        ...
                                        @else
                                            ...
                                            <a class="btn btn-sm btn-warning mr-5" data-toggle="tooltip"
                                                title="@lang('Sửa')" data-original-title="@lang('Sửa')"
