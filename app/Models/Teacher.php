...
class Teacher extends Model
{
    ...
    public static function getSqlTeacher($params = [])
    {
        $query = Teacher::select('admins.*', 'tb_roles.name AS role_name')
            ->when(!empty($params['keyword']), function ($query) use ($params) {
                ...
                $query->where('admins.id', $params['teacher_id']);
            })->when(!empty($params['teacher_type']), function ($query) use ($params) {
                $query->where('admins.teacher_type', $params['teacher_type']);
            })->orderBy('admins.id', 'desc');

        $query->leftJoin('tb_roles', 'admins.role', '=', 'tb_roles.id');
        $query->where('admins.admin_type', Consts::ADMIN_TYPE['teacher']);
    ...
    public static function getSqlTeacherInMonth($params = [])
    {
        $query = Teacher::select('admins.*')
            ->leftJoin('tb_schedules', 'admins.id', '=', 'tb_schedules.teacher_id')
            ->when(!empty($params['keyword']), function ($query) use ($params) {
                $keyword = $params['keyword'];
                return $query->where(function ($where) use ($keyword) {
                    return $where->where('admins.email', 'like', '%' . $keyword . '%')
