    ...
    public function update(Request $request, Order $order)
    {
        ...
        try {
            ...
            if ($order->status == 1) {
                ...
                $syllasbus = Syllabus::find($order->syllabus_id);
                // kiểm tra khóa và tạo
                $courses = UserService::createCourseOnline($syllasbus);
                // kiểm tra lớp online và tạo
                $class = UserService::createClassOnline($customer, $courses, $syllasbus);
                // add học viên vào lớp
                UserService::addStudentClass($customer->id, $class->id);
                // Tạo sẵn lesson user cho từng buổi học
                if(isset($syllasbus->lessons) && $syllasbus->lessons->count() > 0){
                    foreach($syllasbus->lessons as $lesson){
                        UserService::createLessonUsser($customer->id, $lesson->id);
                    }
                }
            }
            DB::commit();
