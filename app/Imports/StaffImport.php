...
class StaffImport implements ToCollection
{
    ...
    public function __construct($params = [])
    {
        set_time_limit(0);
        $this->params = $params;
        $this->idsStaff = Staff::get()->pluck('admin_code');
    }
    ...
    {   
        ...
        try {
            ...
            $commonIds = collect($this->idsStaff)->intersect($filteredIdsRow)->toArray();
            if (!empty($commonIds)) {
                $this->hasError = true;
            ...
            foreach ($rows as $row) {
                ...
                $admin_code_auto = 'CB' . $newNumericPart;
                $json = [
                    "last_name" => $row[3] ?? '',
                    "middle_name" => $row[4] ?? '',
                    "first_name" => $row[5] ?? '',
                ...
                $staff = Staff::create([
                    ...
                    'birthday' => $birthday,
                    'gender' => $row[8],
                    'admin_type' => Consts::ADMIN_TYPE['staff'],
                    'json_params' => $json,
                ]);
            }
            $this->hasError = false;
    ...
    private function isHeaderRow(array $row)
    {
        ...
        $expectedHeaders = ['Mã CBNV*', 'Email', 'Mật khẩu*', 'Họ', 'Tên đệm', 'Tên', 'Địa chỉ', 'Ngày sinh', 'Giới tính', 'Chức vụ'];
