...
class StudentServiceImport implements ToCollection
{
    /**
     * @param array $row
     *
    ...
    public function collection(Collection $rows)
    {
        ...
        try {
            foreach ($rows as $key => $row) {
                ...
                $student_service = StudentService::where('student_id', $student->id)->where('service_id', trim($row[4]))->first();
                if (!empty($student_service)) {
                    $this->rowError++;
                ...
                $student_service = StudentService::create([
                    'student_id' => $student->id,
                    'service_id' => trim($row[4]),
                    'payment_cycle_id' => trim($row[5]),
